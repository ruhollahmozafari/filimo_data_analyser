...
class FilimoLinksSpider(scrapy.Spider):
    ...
    name = 'filimo_links'
    start_urls = [MOVIE_URL + str(page) for page in range(1, 55)] + \
        [SERIE_URL+str(page) for page in range(1, 20)]

    def parse(self, response):
        """find all movie page link and pass it to detail parsers"""

        ic('************* in parse method in spider ************* \n\n')
        ...
        for link in links:  # for each links found in movie/series list, send a request with that link to parse in movie page
            # TODO: just for testing remove the second if
    ...
    def parse_movie_page(self, response):
        ...
        movie_obj = Movie.objects.create(
            ...
            summary=response.css(
                csspath.movie_summary_path).get().replace('\n', ''),
            imdb_rating=response.css(csspath.movie_imdb_rating).get(),
            filimo_rating=response.xpath(
                csspath.movie_filimo_rating_path).get(),
            filimo_total_votes=response.xpath(
                csspath.movie_filimo_total_votes).get(),
